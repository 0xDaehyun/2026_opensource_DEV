...
class LsErrorRoutingTest {
    private MockMvc mockMvcThrowing(RuntimeException failure) {
        ...
        return MockMvcBuilders.standaloneSetup(controller(dispatcher))
    ...
    void routesAnUnknownTrToTheLsEnvelope() throws Exception {
        MockMvc mockMvc = MockMvcBuilders
                .standaloneSetup(controller(new LsTrDispatcher(List.of())))
                .setControllerAdvice(new LsErrorAdvice(new LsErrorMapper()))
                .build();
    ...
    private LsController controller(LsTrDispatcher dispatcher) {
        return new LsController(dispatcher, new TokenController(), Duration.ofHours(24));
    }
